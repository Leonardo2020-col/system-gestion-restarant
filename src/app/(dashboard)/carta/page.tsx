    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Badge variant="secondary">{categorias.length} categorías</Badge>
                ...
                <Card key={p.id}>
                  <CardContent className="p-3 flex gap-3">
                    {p.imagen_url ? (
                      ...
                      <div className="w-16 h-16 rounded-lg bg-muted flex items-center justify-center text-2xl shrink-0">🍽️</div>
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-foreground font-medium text-sm truncate">{p.nombre}</p>
                      {p.descripcion && (
                        <p className="text-muted-foreground text-xs mt-0.5 line-clamp-2">{p.descripcion}</p>
                      )}
